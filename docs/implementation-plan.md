...
- AI HOT 简报转稿：解析固定格式邮件，自动生成并美化可发布每日观察稿、栏目结构、趋势判断、深挖建议和解析检查。
- 简报发布台：支持封面图上传、自动来源图/自动封面兜底、图片出处标注和一键发布本地状态。
...
- 标题：`# AI HOT 每日简报 · YYYY年MM月DD日`
- 元信息：数据来源、时间窗、精选条数
- 分类栏目：模型发布/更新、产品发布/更新、行业动态、论文研究、技巧与观点
- 内容条目：编号、标题、摘要、来源行、原文链接
- 趋势判断
- 深挖建议
...
- 趋势判断段落
- 深挖建议段落
- 封面图：作者上传优先，其次使用邮件中解析到的来源图片，否则生成自动封面
- 图片出处说明
- 来源说明
- 解析检查：条目数量、原文链接、趋势判断、深挖建议
...
1. 将一键发布从本地状态接入真实文章 repository。
2. 实现服务端 `sourceFetcher` adapter：URL 抓取、正文抽取、来源快照、图片元数据、失败分类。
3. 将写稿台草稿一键送入投稿表单。
4. 实现投稿表单和审核队列。
5. 选定 Supabase/Postgres schema。
6. 实现点赞、收藏、关注的持久化状态更新。
7. 增加组件和页面测试。
